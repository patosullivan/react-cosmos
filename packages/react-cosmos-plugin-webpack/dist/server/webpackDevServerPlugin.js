...
export async function webpackDevServerPlugin({ cosmosConfig, platform, expressApp, sendMessage, }) {
    if (platform !== 'web')
        return;
    const userWebpack = getWebpack(cosmosConfig.rootDir);
    if (!userWebpack)
        return;
    const webpackConfig = (await getDevWebpackConfig(cosmosConfig, userWebpack));
    ...
    if (cosmosConfig.staticPath === null) {
        ...
        if (webpackDerivedStaticPath !== null) {
            serveStaticDir(expressApp, path.resolve(cosmosConfig.rootDir, webpackDerivedStaticPath), cosmosConfig.publicUrl);
        }
    }
    ...
    const wdmInst = wdmModule.default(webpackCompiler, {
...
function getWebpackStaticPath({ devServer }) {
