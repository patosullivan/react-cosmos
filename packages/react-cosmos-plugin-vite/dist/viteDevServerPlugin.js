import { startFixtureWatcher } from 'react-cosmos';
import { pickRendererUrl } from 'react-cosmos-core';
import { createServer } from 'vite';
import { createCosmosViteConfig } from './createCosmosViteConfig.js';
import { reactCosmosViteRollupPlugin, userImportsResolvedModuleId, } from './reactCosmosViteRollupPlugin.js';
export async function viteDevServerPlugin({ cosmosConfig, platform, }) {
    if (platform !== 'web')
        return;
    ...
    const watcher = await startFixtureWatcher(cosmosConfig, 'add', () => {
        const module = server.moduleGraph.getModuleById(userImportsResolvedModuleId);
        if (!module) {
            throw new Error(`Vite module graph doesn't contain module with id ${userImportsResolvedModuleId}`);
        }
